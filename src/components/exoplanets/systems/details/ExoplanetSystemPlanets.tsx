...
export const ExoplanetSystemPlanets = ({data, t}: Props) => {
    return (
        <section className="relative overflow-hidden rounded-[2.2rem] border border-[var(--color-border)] bg-[var(--color-card)] p-5 shadow-[var(--shadow-card)]">
            ...
            <div className="relative z-10">
                <div className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <h2 className="mt-2 bg-gradient-to-r from-[var(--color-text)] via-[var(--color-accent)] to-[var(--color-brand-secondary)] bg-clip-text text-3xl font-black uppercase tracking-[-0.06em] text-transparent">
                            {data.system.hostname ?? t.unknown}
                        </h2>
                    </div>

                    <p className="rounded-full border border-[var(--color-border)] bg-[var(--color-glass)] px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--color-text-muted)]">
                        ...
                        {data.planets.map((planet, index) => {
                            ...
                            return (
                                <article
                                    ...
                                    className="relative overflow-hidden rounded-[1.6rem] border border-[var(--color-border)] bg-[rgba(5,12,28,.62)] p-4 transition hover:border-[var(--color-accent)] hover:shadow-[var(--shadow-glow)]"
                                >
                                    <div className="pointer-events-none absolute right-[-42px] top-[-42px] h-[150px] w-[150px] rounded-full opacity-35" style={{background: getPlanetGradient(planet.pl_eqt)}} />
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <h3 className="mt-3 text-xl font-black uppercase tracking-[-0.05em] text-[var(--color-text)]">
                                        ...
                                        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                                            <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--color-text-muted)]">
                                            ...
                                            <Link
                                                href={href}
                                                className="inline-flex items-center gap-2 rounded-full border border-[var(--color-accent)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--color-accent)] transition hover:bg-[var(--color-accent)] hover:text-black"
                                            >
                                                {t.open}
...
const PlanetMetric = ({
    ...
    <div className="rounded-[1rem] border border-[var(--color-border)] bg-[var(--color-card)]/50 p-3">
