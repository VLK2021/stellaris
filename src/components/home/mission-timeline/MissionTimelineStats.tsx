...
export const MissionTimelineStats = ({stats}: Props) => {
    return (
        <div className="mt-6 grid gap-3 rounded-[1.45rem] border border-white/10 bg-white/[0.03] p-3 backdrop-blur-xl md:grid-cols-2 xl:grid-cols-5">
            {stats.map((stat, index) => {
                const Icon = icons[index] ?? Rocket;

                return (
                    <div key={stat.label} className="flex items-center gap-3 border-white/10 xl:border-r xl:last:border-r-0">
                        <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-cyan-300/15 bg-cyan-300/8">
