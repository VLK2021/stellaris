...
const EnergyBeam = ({className, reverse}: EnergyBeamProps) => (
    <div
        className={`absolute z-10 h-[2px] w-[178px] origin-left overflow-hidden bg-cyan-300/10 ${className}`}
    >
        <motion.div
...
const CircleNode = ({className, title, value, subtitle, glow}: CircleNodeProps) => (
    <motion.div
        ...
        className={`absolute z-20 rounded-full border border-white/10 shadow-[0_18px_65px_rgba(0,0,0,0.26)] backdrop-blur-2xl ${className}`}
        style={{
