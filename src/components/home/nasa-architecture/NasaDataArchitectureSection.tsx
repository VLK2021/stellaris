...
export const NasaDataArchitectureSection = ({locale}: Props) => {
    ...
    return (
        <section className="relative isolate overflow-hidden bg-[#020611] px-4 py-16 text-white sm:px-6 lg:px-10">
            <div className="relative mx-auto max-w-[1680px] overflow-hidden rounded-[2.8rem] border border-white/10 p-5 sm:p-8 lg:p-10">
                ...
                <div className="relative z-10">
                    ...
                    >
                        ...
                        <h2 className="mt-4 text-4xl font-black tracking-[-0.07em] sm:text-6xl">
                            {locale.title}
                        </h2>
                    ...
                    <div className="relative mt-10 grid gap-5 lg:min-h-[540px] lg:grid-cols-[1fr_300px_1fr]">
                        <div className="grid gap-5">{leftNodes.map((node, i) => <NasaArchitectureNode key={node.id} node={node} locale={locale} index={i} />)}</div>
                        <div className="relative flex items-center justify-center"><NasaArchitectureOrbit /><NasaArchitectureCore title={locale.coreTitle} subtitle={locale.coreSubtitle} /></div>
                        <div className="grid gap-5">{rightNodes.map((node, i) => <NasaArchitectureNode key={node.id} node={node} locale={locale} index={i + 4} />)}</div>
                    </div>
