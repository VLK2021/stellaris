...
import type {EarthOfficialSourceDetails} from "@/src/types/earth/earthEnrichment.types";
...
export const EarthEventSources = ({sources, officialSources, t}: Props) => {
    return (
        <motion.section
        ...
        >
            ...
            <h2 className="relative z-10 text-xl font-black uppercase tracking-[-0.04em]">
                {t.details.sources}
            </h2>

            <div className="relative z-10 mt-4 grid gap-3">
                {sources.map((source, index) => {
                    const official = officialSources.find((item) => item.id === source.id);
                        ...
                        >
                            <div className="flex items-start justify-between gap-4">
                                <div className="min-w-0">
                                    <p className="earth-label text-sm font-black">
                                        {source.id}
                                    </p>

                                    <h3 className="mt-1 text-base font-black">
                                        {official?.title ?? source.id}
                                    </h3>
