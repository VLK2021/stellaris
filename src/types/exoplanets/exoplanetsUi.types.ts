export type ExoplanetsLocale = {
    hero: {
        eyebrow: string;
        title: string;
        description: string;
    };
    portal: {
        eyebrow: string;
        title: string;
        description: string;
        open: string;
        online: string;
        catalog: {
            title: string;
            description: string;
        };
        systems: {
            title: string;
            description: string;
        };
        stars: {
            title: string;
            description: string;
        };
        methods: {
            title: string;
            description: string;
        };
        atmospheres: {
            title: string;
            description: string;
        };
        habitable: {
            title: string;
            description: string;
        };
    };
    details: {
        back: string;
        eyebrow: string;
        generated: string;
        confirmed: string;
        orbiting: string;
        dataSource: string;
        discoveryYear: string;
        method: string;
        orbitPeriod: string;
        equilibriumTemp: string;
        physicalParameters: string;
        orbitalParameters: string;
        hostStar: string;
        discoveryAndSystem: string;
        radiusEarth: string;
        radiusJupiter: string;
        massEarth: string;
        massJupiter: string;
        density: string;
        insolation: string;
        semiMajorAxis: string;
        eccentricity: string;
        inclination: string;
        transitDepth: string;
        transitDuration: string;
        spectralType: string;
        starTemperature: string;
        starRadius: string;
        starMass: string;
        starAge: string;
        facility: string;
        telescope: string;
        instrument: string;
        distance: string;
        starsInSystem: string;
        planetsInSystem: string;
        loadingError: string;
        notFound: string;
        unknown: string;
    };
    catalog: {
        eyebrow: string;
        title: string;
        description: string;
        search: string;
        searchPlaceholder: string;
        discoveryMethod: string;
        all: string;
        sortBy: string;
        order: string;
        desc: string;
        asc: string;
        discoveryYear: string;
        planetName: string;
        distance: string;
        radius: string;
        mass: string;
        temperature: string;
        total: string;
        page: string;
        loadingError: string;
        unknownError: string;
        unknownPlanet: string;
        unknownMethod: string;
        hostStar: string;
        orbit: string;
        details: string;
        backToExoplanets: string;
    };

    systems: {
        backToExoplanets: string;
        eyebrow: string;
        title: string;
        description: string;
        search: string;
        searchPlaceholder: string;
        sortBy: string;
        order: string;
        asc: string;
        desc: string;
        hostName: string;
        distance: string;
        planets: string;
        stars: string;
        moons: string;
        temperature: string;
        radius: string;
        mass: string;
        age: string;
        spectralType: string;
        metallicity: string;
        gravity: string;
        total: string;
        page: string;
        open: string;
        unknown: string;
        loadingError: string;
        unknownError: string;
        backToSystems: string;
        overview: string;
        hostStar: string;
        systemArchitecture: string;
        stellarPhysics: string;
        detectedPlanets: string;
        detectedStars: string;
        detectedMoons: string;
        source: string;
        noPlanets: string;
        planetName: string;
        discoveryYear: string;
        method: string;
        orbitPeriod: string;
        equilibriumTemp: string;
        loading: string;
        notFound: string;
    };
    habitable: {
        backToExoplanets: string;
        backToHabitable: string;
        eyebrow: string;
        title: string;
        description: string;
        search: string;
        searchPlaceholder: string;
        total: string;
        page: string;
        worlds: string;
        score: string;
        distance: string;
        year: string;
        radius: string;
        mass: string;
        temperature: string;
        orbit: string;
        hostStar: string;
        open: string;
        temperateZone: string;
        hotZone: string;
        coldZone: string;
        unknownZone: string;
        strongCandidate: string;
        moderateCandidate: string;
        weakCandidate: string;
        loadingError: string;
        unknownError: string;
        unknownWorld: string;
        overview: string;
        habitabilityProfile: string;
        planetaryParameters: string;
        stellarContext: string;
        discoveryContext: string;
        habitabilityScore: string;
        habitabilityZone: string;
        dataSource: string;
        discoveryMethod: string;
        discoveryYear: string;
        equilibriumTemp: string;
        orbitalPeriod: string;
        stellarTemperature: string;
        stellarRadius: string;
        stellarMass: string;
        notFound: string;
    };
    atmospheres: {
        backToExoplanets: string;
        backToAtmospheres: string;
        eyebrow: string;
        title: string;
        description: string;
        search: string;
        searchPlaceholder: string;
        sortBy: string;
        order: string;
        asc: string;
        desc: string;
        total: string;
        page: string;
        planetName: string;
        hostName: string;
        equilibriumTemp: string;
        radius: string;
        mass: string;
        density: string;
        orbit: string;
        insolation: string;
        stellarTemperature: string;
        stellarRadius: string;
        stellarMass: string;
        atmosphereScore: string;
        atmosphereClass: string;
        atmosphereProfile: string;
        candidateType: string;
        planetParameters: string;
        stellarContext: string;
        temperateClass: string;
        warmClass: string;
        hotClass: string;
        coldClass: string;
        unknownClass: string;
        rockyCandidate: string;
        subNeptune: string;
        gasGiant: string;
        lowDensity: string;
        unknownCandidate: string;
        details: string;
        detailsDescription: string;
        openPlanetCatalog: string;
        unknown: string;
        loadingError: string;
        unknownError: string;
        notFound: string;
        distance: string;
        discoveryYear: string;
    };

    methods: {
        backToExoplanets: string;
        eyebrow: string;
        title: string;
        description: string;
        search: string;
        searchPlaceholder: string;
        sortBy: string;
        order: string;
        asc: string;
        desc: string;
        total: string;
        page: string;
        method: string;
        discoveryMethod: string;
        planets: string;
        systems: string;
        firstYear: string;
        lastYear: string;
        avgDistance: string;
        avgRadius: string;
        avgMass: string;
        avgTemperature: string;
        methodStats: string;
        confirmedPlanets: string;
        details: string;
        transitDescription: string;
        radialVelocityDescription: string;
        imagingDescription: string;
        microlensingDescription: string;
        timingDescription: string;
        astrometryDescription: string;
        otherDescription: string;
        loadingError: string;
        unknownError: string;
        backToMethods: string;
        advantages: string;
        limitations: string;
        knownExamples: string;
        openPlanetCatalog: string;
        notFound: string;

        transitProOne: string;
        transitProTwo: string;
        transitProThree: string;
        transitConOne: string;
        transitConTwo: string;

        radialVelocityProOne: string;
        radialVelocityProTwo: string;
        radialVelocityProThree: string;
        radialVelocityConOne: string;
        radialVelocityConTwo: string;

        imagingProOne: string;
        imagingProTwo: string;
        imagingProThree: string;
        imagingConOne: string;
        imagingConTwo: string;

        generalProOne: string;
        generalProTwo: string;
        generalProThree: string;
        generalConOne: string;
        generalConTwo: string;
    };
};